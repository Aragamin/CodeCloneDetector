...
def factorial(n):
    ...
    if n == 0:
        ...
        print(f"Вычисление факториала для {n}")  # Добавленная строка 1
...
def fibonacci(n):
    ...
    if n <= 0:
        ...
        print(f"Вычисление числа Фибоначчи для {n}")  # Добавленная строка 2
...
def is_prime(n):
    ...
    for i in range(2, int(n ** 0.5) + 1):
        if n % i == 0:
            return False
    print(f"Проверка числа {n} на простоту")  # Добавленная строка 3
    return True
...
def quicksort(arr):
    ...
    if len(arr) <= 1:
        ...
        print(f"Сортировка: {arr}")  # Добавленная строка 4
